...
const PlayerContent: React.FC<PlayerContentProps> = ({ song, songUrl }) => {
  ...
  const { play, pause, load, volume, setVolume, mute } = useAudioPlayer();
  const [ volumeAudio, setVolumeAudio ] = useState(1);
  const [ isPlaying, setIsPlaying ] = useState(false);

  const Icon = isPlaying ? BsPauseFill : BsPlayFill;
  const VolumeIcon = volumeAudio === 0 ? HiSpeakerXMark : HiSpeakerWave;
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 h-full">
      ...
      <div className="hidden h-full md:flex justify-center items-center w-full max-w-[722px] gap-x-6">
        ...
        <div onClick={handlePlay} className="flex items-center justify-center h-10 w-10 rounded-full bg-white p-2 cursor-pointer">
          <Icon size={30} className="text-black"/>
        </div>
      ...
      <div className="hidden md:flex w-full justify-end pr-2">
        <div className='flex items-center gap-x-2 w-[120px]'>
          ...
          <Slider
            value={volume}
            onChange={(value) => {setVolumeAudio(value); setVolume(value)}}
          />
        </div>
      </div>
    </div>
